...
SELECT
    ...
    CASE strftime('%m', order_purchase_timestamp)
    	...
    	ELSE 'Unknown'
  	END AS month,
    AVG(CASE WHEN year = '2016' THEN julianday(order_delivered_customer_date) - julianday(order_purchase_timestamp) ELSE NULL END) AS Year2016_real_time,
    AVG(CASE WHEN year = '2017' THEN julianday(order_delivered_customer_date) - julianday(order_purchase_timestamp) ELSE NULL END) AS Year2017_real_time,
    AVG(CASE WHEN year = '2018' THEN julianday(order_delivered_customer_date) - julianday(order_purchase_timestamp) ELSE NULL END) AS Year2018_real_time,
    AVG(CASE WHEN year = '2016' THEN julianday(order_estimated_delivery_date) - julianday(order_purchase_timestamp) ELSE NULL END) AS Year2016_estimated_time,
    AVG(CASE WHEN year = '2017' THEN julianday(order_estimated_delivery_date) - julianday(order_purchase_timestamp) ELSE NULL END) AS Year2017_estimated_time,
    AVG(CASE WHEN year = '2018' THEN julianday(order_estimated_delivery_date) - julianday(order_purchase_timestamp) ELSE NULL END) AS Year2018_estimated_time
FROM DeliveryTime
GROUP BY month_no
ORDER BY month_no;