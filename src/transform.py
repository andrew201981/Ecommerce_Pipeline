...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    merged_items_orders = pd.merge(items, orders, on='order_id')
    data = pd.merge(merged_items_orders, products, on='product_id')
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    mask_2017 = orders['order_purchase_timestamp'].dt.year == 2017
    filtered_dates = orders[mask_2017]

    # Counting the orders per day.

    order_purchase_ammount_per_date = filtered_dates.groupby(filtered_dates['order_purchase_timestamp'].dt.date)['order_id'].count().reset_index()
    ...
    result_df = order_purchase_ammount_per_date.merge(holidays, left_on='date', right_on='date', how='left')
    result_df = result_df[["order_count","date"]]
